...
export default function InsightModal({ insightType, businessData, onClose }) {
  ...
  if (businessData.researchReport) {
    return (
      <div className="insight-overlay">
        <div className="insight-modal glass-panel" style={{ overflowY: 'auto', maxHeight: '90vh', width: '800px', maxWidth: '90vw' }}>
          <button className="modal-close" onClick={onClose}><X size={24} /></button>
          <div className="insight-hero mb-6">
            <TrendingUp size={48} className="text-cyan mb-4" />
            <h2>Live AI Engine Market Research</h2>
            <p className="text-secondary">Extracted via CrewAI backend pipeline.</p>
          </div>
          <div style={{ whiteSpace: 'pre-wrap', lineHeight: '1.8', fontSize: '15px', color: '#e2e8f0', background: 'rgba(0,0,0,0.2)', padding: '24px', borderRadius: '12px' }}>
            {businessData.researchReport}
          </div>
        </div>
      </div>
    );
